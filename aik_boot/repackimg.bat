...
echo kernel = %kernelname% & set "kernel=split_img/%kernelname%" >nul 2>nul
:skipzimg
...
echo ramdisk = %ramdiskname% >nul 2>nul
...
echo cmdline = %cmdline% >nul 2>nul
goto skipaosp
...
echo type = %arch% %os% %type% (%comp%) >nul 2>nul
...
echo kernel_offset = %kerneloff% >nul 2>nul
...
echo ramdisk_offset = %ramdiskoff% >nul 2>nul
...
:skipsoff
echo second_offset = %secondoff% >nul 2>nul
...
:skiptoff
echo tags_offset = %tagsoff% >nul 2>nul
...
echo dtb_offset = %dtboff% >nul 2>nul
:skipdoff
...
echo os_version = %osver% >nul 2>nul
:skiposver
...
echo os_patch_level = %oslvl% >nul 2>nul
:skiposlvl
...
echo header_version = %hdrver% >nul 2>nul
:skiphdrver
...
echo hashtype = %hashtype% & set "hashtype=--hashtype %hashtype%" >nul 2>nul
:skiphash
